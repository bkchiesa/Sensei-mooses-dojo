#!/usr/bin/env python3
"""Snap magenta chroma and slice starter ultimate sheets into frames."""
...
REPO = ROOT.parent
ART_IDS = ("matt", "simon", "rich", "amanda", "jb")
COLS, ROWS = 6, 2
TARGET_CHROMA = (255, 0, 255)
SRC_DIR = Path("/opt/cursor/artifacts/assets")
ARTIFACT_STARTER = Path("/opt/cursor/artifacts/starter-ults")
...
def is_chroma(r: int, g: int, b: int) -> bool:
    ...
    if abs(r - b) > 55:
        return False
    # Background magentas stay high-R/high-B with G not dominating.
    if g > min(r, b) - 8:
        return False
